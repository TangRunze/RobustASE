...
\begin{tikzpicture}[
  ...
  \draw[tofrom] (MLE) -- node[midway,left] {$\mathrm{For}$ $\mathrm{any}$ $\mathrm{fixed}$ $m$, \\$\mathrm{any}$ $1 \le i, j \le n$, \\$\underset{n \to \infty}{\lim} \mathrm{Bias}(\hat{P}_{ij}^{(1)}) =$\\$ \underset{n \to \infty}{\lim} \mathrm{Bias}(\widetilde{P}_{ij}^{(1)})$\\$\mathrm{Var}(\widetilde{P}_{ij}^{(1)})/\mathrm{Var}(\hat{P}_{ij}^{(1)})$\\$ = O(m n^{-1} (\log n)^3)$} (XMLE);
...
\]
\RT{So in order to have the inequality to be small, we need the rank $k$ increase of order $o(n)$?}
...
\RT{Here we need $k = O(n^{1/2-\epsilon})$?}
...
\[
	\| U^T \hat{U} - W_1 W_2^T \|_F = O(k n^{-1} \log n)
\]
...
\[
	\| W^* \hat{S} - S W^* \|_F = O(k^{3/2} \log n),
\]
\[
	\|W^* \hat{S}^{1/2} - S^{1/2} W^* \|_F = O(k^{3/2} n^{-1/2} \log n)
\]
and
\[
	\| W^* \hat{S}^{-1/2} - S^{-1/2} W^* \|_F = O(k^{3/2} n^{-3/2} \log n)
\]
with high probability.
\end{lemma}
\begin{proof}
...
\[
\|\hat{U} - U W\|_F^2 \le \frac{2 \|\hat{U} \hat{U}^T - U U^T\|_F^2}{\delta^2} = O(k n^{-1/2} \sqrt{\log n}).
\]
...
\begin{equation}
\label{eqn:QFnormL1}
\| Q \|_F = \| \hat{U} - U U^T \hat{U} \|_F \le \| \hat{U} - U T \|_F = O(k n^{-1/2} \sqrt{\log n}).
\end{equation}
...
\begin{align*}
    ...
    \le & O(k \log n) + O(k^{3/2} \log n) + \| U^T (\hat{P} - H) U \|_F
\end{align*}
...
\[
	\|\hat{Z} - Z W\|_F = k \| (\hat{P} - H) U S^{-1/2}\|_F + O(k^{5/2} n^{-1/2} (\log n)^{3/2})
\]
with high probability.
\end{lemma}
\begin{proof}
...
\[
	\|Q_1\|_F \le \| U\|_F \| U^T \hat{U} - W^* \|_F = O(k^{3/2} n^{-1} \log n).
\]
By Lemma \ref{lemma:exchangeL1},
\[
	\|Q_2\|_F = O(k^{3/2} n^{-1/2} \log n).
\]
By Equation (\ref{eqn:QFnormL1}),
if we assume $k = O(n^{1/2-\epsilon})$,
\[
	\|Q_3\|_F \le \|Q\|_F + \|Q_1\|_F = O(k n^{-1/2} (\log n)^{1/2}).
\]
otherwise,
\[
	\|Q_3\|_F \le \|Q\|_F + \|Q_1\|_F = O(k^{3/2} n^{-1/2} (\log n)^{1/2}).
\]
By Equation (\ref{eqn:uPhatdiffHuL1}),
\[
	\| U U^T(\hat{P} - H)U W^*\hat{S}^{-1/2} \|_F
    \le k^{3/2} \| U^T(\hat{P} - H)U \|_F \| \hat{S}^{-1/2} \|_2
    = O(k^{5/2} n^{-1/2} \log n).
\]
By Lemma \ref{lemma:exchangeL1},
\[
...
\begin{align*}
	& \| \hat{Z} - U S^{1/2} W^* \|_F \\
    = & \| (\hat{P} - H) U W^* \hat{S}^{-1/2} \|_F + O(k^{5/2} n^{-1/2} \log n)
    + \|I - U U^T \|_2 \| \hat{P} - H \|_2 O(k^{3/2} n^{-1} (\log n)^{1/2}) \\
    & + O(k^{3/2} n^{-1/2} \log n) + O(k^2 n^{-1/2} \log n)\\
    = & \| (\hat{P} - H) U W^* \hat{S}^{-1/2} \|_F + O(k^{5/2} n^{-1/2} \log n) \\
    \le & \| (\hat{P} - H) U S^{-1/2} W^* \|_F + \|(\hat{P} - H) U (W^* \hat{S}^{-1/2} - S^{-1/2} W^*) \|_F + O(k^{5/2} n^{-1/2} \log n) \\
    = & k \| (\hat{P} - H) U S^{-1/2}\|_F + O(k^2 n^{-1} (\log n)^{3/2}) + O(k^{5/2} n^{-1/2} \log n) \\
    = & k \| (\hat{P} - H) U S^{-1/2}\|_F + O(k^{5/2} n^{-1/2} (\log n)^{3/2}).
\end{align*}
\RT{$\|I - U U^T\|_2 = O(1)$}
...
There exists a rotation matrix $W$ such that for sufficiently large $n$,
\[
	\max_i \| \hat{Z}_i - W Z_i \|_2 = O(k^{5/2} n^{-1/2} (\log n)^{3/2})
\]
with high probability.
\end{theorem}
\begin{proof}
...
\[
	\|\hat{Z} - Z W\|_F = \| (\hat{P} - H) U S^{-1/2} \|_F + O(k^{5/2} n^{-1/2} (\log n)^{3/2})
\]
and similarly we could have the bound for each column vector \RT{Similarly}
\begin{align*}
	\max_i \| \hat{Z}_i - W Z_i \|_2
    \le & \frac{1}{\lambda_k^{1/2}(H)} \max_i \| ((\hat{P} - H) U)_i \|_2 + O(k^{5/2} n^{-1/2} (\log n)^{3/2}) \\
    \le & \frac{k^{1/2}}{\lambda_k^{1/2}(H)} \max_j \| (\hat{P} - H) u_j \|_{\infty} + O(k^{5/2} n^{-1/2} (\log n)^{3/2})
\end{align*}
...
\begin{align*}
	\max_i \| \hat{Z}_i - W Z_i \|_2
    & \le \frac{C k^{1/2}}{\lambda_k^{1/2}(H)} (\log n)^{3/2} + O(k^{5/2} n^{-1/2} (\log n)^{3/2}) \\
    & = O(k^{5/2} n^{-1/2} (\log n)^{3/2}).
\end{align*}
\end{proof}
...
$\left|  \hat{Z}_i^T \hat{Z}_j - Z_i^T Z_j \right| = O(k^{5} n^{-1} (\log n)^{3})$ with high probability.
\end{lemma}
\begin{proof}
...
\begin{align*}
    \left|  \hat{Z}_i^T \hat{Z}_j - Z_i^T Z_j \right|
    = & (\|\hat{Z}_i\|_2 + \|Z_j\|_2) O(k^{5/2} n^{-1/2} (\log n)^{3/2}) \\
    \le & (\|\hat{Z}_i - W Z_i\|_2 + \|W Z_i\|_2 + \|Z_j\|_2) O(k^{5/2} n^{-1/2} (\log n)^{3/2}) \\
    = & O(k^{5} n^{-1} (\log n)^{3})
\end{align*}
with high probability.
\end{proof}
...
\begin{theorem}
\label{thm:VarASEL1}
$\mathrm{Var}(\hat{Z}_i^T \hat{Z}_j) = O(k^{10} n^{-2} (\log n)^6)$ with high probability.
\end{theorem}
\begin{proof}
...
\begin{align*}
    ...
    = & O(k^{10} n^{-2} (\log n)^6)
\end{align*}
with high probability. \RT{Expectation with high probability?}
\end{proof}
...
\begin{align*}
	\max_i \| \hat{Z}_i - W Z_i \|_2
    \le & \frac{1}{\lambda_d^{1/2}(H)} \max_i \| ((\hat{P} - H) U)_i \|_2 + O(k^{5/2} n^{-1/2} (\log n)^{3/2}) \\
    \le & \frac{d^{1/2}}{\lambda_d^{1/2}(H)} \max_j \| (\hat{P} - H) u_j \|_{\infty} + O(k^{5/2} n^{-1/2} (\log n)^{3/2})
\end{align*}
...
\[
	\max_i \| \hat{Z}_i - W Z_i \|_2
    \le \frac{C k^{1/2}}{\lambda_d^{1/2}(H)} (\log n)^{3/2} + O(k^{5/2} n^{-1/2} (\log n)^{3/2})
    = O(k^{5/2} n^{-1/2} (\log n)^{3/2}).
\]
\end{proof}
