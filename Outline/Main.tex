...
\begin{tikzpicture}[
  ...
  \matrix{
  	\& \node[block] (Data) {Data $G^{(1)}, \cdots, G^{(m)}$};\\
...
Under each of the models mentioned in Section \ref{section:model}, our goal is to estimate the parameter matrix $P$ based on the $m$ observations $A^{(1)}, \cdots, A^{(m)}$. In this section, we present four estimators as in Figure \ref{fig:roadmap}, the standard entry-wise MLE $\hat{P}^{(1)}$, the low-rank approximation of the entry-wise MLE $\widetilde{P}^{(1)}$, the entry-wise robust estimator ML$q$E $\hat{P}^{(q)}$, and the low-rank approximation of the entry-wise ML$q$E $\widetilde{P}^{(q)}$.
...
\begin{itemize}
\item Motivated by the low-rank structure of the parameter matrix $P$ in WRDPG, we use the similar idea as in \cite{chatterjee2015matrix, tang2016law} to propose the estimate $\widetilde{P}^{(1)}$ based on the spectral decomposition of $\hat{P}^{(1)}$. we use the adjacency spectral embedding (ASE) studied by Sussman et. al. to enforce a low-rank approximation on the entry-wise MLE matrix $\hat{P}^{(1)}$, which will decrease the variance without losing much in bias if we embed it into the right dimension.
