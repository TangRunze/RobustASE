...
\begin{align*}
	...
	\le & O(n^{-2} (\log n)^6) + \frac{2}{n^2} E[\hat{P}_{ij}^2 \mathbb{I}\{\hat{P}_{ij} \le a\}] + \frac{2}{n^2} E[(\hat{P}_{ij} + M)^2 \mathbb{I}\{\hat{P}_{ij} \le a\}] \\
	\le & O(n^{-2} (\log n)^6) + \frac{2 a^2}{n^2} + \frac{2 (a+M)^2}{n^2} \\
	\le & O(n^{-2} (\log n)^6) + \frac{4}{n^2} (a + M)^2
\end{align*}
...
\begin{align*}
	...
	\le & 2 e^{-a/R} \left( 2a^2 + M^2 + 2MR + 4R^2 + 2a(M+ 2R) \right) \\
	\le & 4 e^{-a/R} (a + M + 2R)^2
\end{align*}
Thus,
\[
	\mathrm{Var}((\hat{Z}_i^T \hat{Z}_j)_{\mathrm{tr}}) \le
	O(n^{-2} (\log n)^6) + 16 (a+M+2R)^2 (\frac{1}{n^2} + e^{-a/R}).
\]
Let $a = n^{\epsilon}$ for any $\epsilon > 0$, then
\begin{align*}
	\mathrm{Var}((\hat{Z}_i^T \hat{Z}_j)_{\mathrm{tr}})
