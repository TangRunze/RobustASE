...
\title{Robust LLG Proof}

\author{Runze, Minh, Carey, Jovo, ...}
...
Consider a positive semi-definite stochastic blockmodel parameterized by $B \in [0,R]^{K\times K}$, and $\rho \in (0, 1)^{n}$ with $\sum_{i = 1}^n \rho_i = 1$. Note that $K$ is the number of blocks, and $R > 0$ is an upper bound for the parameter. Assuming that $\mathrm{rank}(B) = d$, then there exists $\nu \in \mathbb{R}^{K\times d}$ such that $B = \nu \nu^T$. Let $\tau$ represent the block membership of each vertex, i.e. $\tau(i) = k$ means vertex $i$ belongs to block $k$. Then the probability matrix $P \in [0, R]^{n\times n}$ can be written as $X X^T$, where $X = [X_1, \cdots, X_n]^T \in \mathbb{R}^{n\times d}$ and $X_i = \nu_{\tau_i}$.

Then we can sample $m$ conditionally i.i.d. symmetric and hollow graphs $G^{(1)}, \cdots, G^{(m)}$, such that conditioned on $X$, $G^{(t)}_{ij} \stackrel{ind}{\sim} \mathrm{Exp}(P_{ij})$ for each $1 \le t \le m$, $1 \le i < j \le n$.
...
Similarly, consider another positive semi-definite stochastic blockmodel parameterized by $B' \in [0, R]^{K' \times K'}$, and $\rho' \in (0, 1)^{n}$ with $\sum_{i = 1}^n \rho_i' = 1$. Note that $K'$ is the number of blocks which is not necessarily equal to $K$, but $R$ is the same upper bound for the parameter. Assuming that $\mathrm{rank}(B') = d'$, then there exists $\nu' \in \mathbb{R}^{K'\times d'}$ such that $B' = \nu' \nu'^{T}$. Let $\tau'$ represent the block membership of each vertex, i.e. $\tau'(i) = k$ means vertex $i$ belongs to block $k$. Then the probability matrix $C \in [0, R]^{n\times n}$ can be written as $Y Y^T$, where $Y = [Y_1, \cdots, Y_n]^T \in \mathbb{R}^{n\times d'}$ and $Y_i = \nu'_{\tau'_i}$.

Then we can sample $m$ conditionally i.i.d.~symmetric and hollow graphs $A^{(1)}, \cdots, A^{(m)}$ such that conditioning on $X$ and $Y$, $A^{(t)}_{ij} \stackrel{ind}{\sim} (1-\epsilon) \mathrm{Exp}(P_{ij}) + \epsilon \mathrm{Exp}(C_{ij})$ for each $1 \le t \le m$, $1 \le i < j \le n$. 
...
\end{lemma}
\noindent \textbf{Require:} Exponential distribution, ML$q$E \\
\begin{proof}
The MLE is
\[
	\hat{P}^{(1)}(x) = \bar{x}.
...
For any $a > 0$, we have
\[
	\sup_{\theta \in [a, R]} \left| \frac{1}{m} \sum_{i=1}^m e^{-\frac{(1-q) X_i}{\theta}}(X_i - \theta) - E_F[e^{-\frac{(1-q) X}{\theta}}(X - \theta)] \right| \stackrel{a.s.}{\to} 0.
\]
\end{lemma}
\noindent \textbf{Require:} Exponential distribution, ML$q$E. Should be easy to extend \\
\begin{proof}
Define $g(x,\theta) = e^{-\frac{(1-q) x}{\theta}}(x - \theta)$ and $d(x) = e^{-\frac{(1-q)x}{R}}(x + R)$. Then $E_F[d(X)] < \infty$ and $g(x,\theta) \le d(x)$ for all $\theta \in [a, R]$.
Combined with the fact that $[a, R]$ is compact and the function $g(x,\theta)$ is continuous at each $\theta$ for all $x > 0$ and measurable function of $x$ at each $\theta$, we have the uniform convergence by Lemma 2.4 in \cite{newey1994large}.
\end{proof}
...
$E[\hat{P}_{ij}^{(q)}] \to \theta(F_{ij})$ as $m \to \infty$, where $F_{ij}$ is the contaminated distribution $(1-\epsilon) \mathrm{Exp}(P_{ij}) + \epsilon \mathrm{Exp}(C_{ij})$. 
\end{lemma}
\begin{proof}
{\color{red}NEED PROOF HERE}
\end{proof}
...
For ML$q$E, {\color{red}NEED PROOF HERE}
\end{proof}
...
\noindent \textbf{Require:} Exponential distribution \\
\noindent
\begin{proof}
...
\noindent \textbf{Require:} Theorem \ref{thm:P1Diff} and assumptions above\\
\begin{proof}
...
\[
|\lambda_i(H) - n \lambda_i(Z_1 Z_1^T)| \le \|Z^T Z - n E[Z_1 Z_1^T]\|_2 = O(\sqrt{n\log{(1/\eta)}})
\]
...
\end{lemma}
\noindent \textbf{Require:} Theorem \ref{thm:P1Diff} and Lemma \ref{lemma:eigSShatL1} \\
\begin{proof}
...
with high probability.
\end{lemma}
\noindent \textbf{Require:} Theorem \ref{thm:P1Diff}, Lemma \ref{lemma:eigSShatL1}, Lemma \ref{lemma:AlmostOrthogonalL1}, Exponential distribution \\
\begin{proof}
...
with high probability.
\end{lemma}
\noindent \textbf{Require:} Lemma \ref{lemma:AlmostOrthogonalL1}, Lemma \ref{lemma:exchangeL1} \\
\begin{proof}
...
with high probability.
\end{theorem}
\noindent \textbf{Require:} Lemma \ref{lemma:XhatDiffXWexpressionL1}, Exponential distribution \\
\begin{proof}
...
\end{lemma}
\noindent \textbf{Require:} Theorem \ref{thm:XhatDiffXWL1} \\
\begin{proof}
...
\]
\end{corollary}
\noindent \textbf{Require:} Lemma \ref{lemma:1stMomentPhatDiffL1}, Exponential distribution \\
\begin{proof}
...
\begin{align*}
	...
	\le & O(m^{-1/2} n^{-1} (\log n)^3) + n^{-c} E[\hat{P}_{ij} \mathbb{I}\{\hat{P}_{ij} \le a\}| \mathrm{Lemma\ } \ref{lemma:1stMomentPhatDiffL1} \mathrm{\ does\ not\ hold}] \\
	& + n^{-c} E[(\hat{P}_{ij} + R) \mathbb{I}\{\hat{P}_{ij} \le a\}| \mathrm{Lemma\ } \ref{lemma:1stMomentPhatDiffL1} \mathrm{\ does\ not\ hold}] \\
	\le & O(m^{-1/2} n^{-1} (\log n)^3) + a n^{-c} + (a+R) n^{-c} \\
	\le & O(m^{-1/2} n^{-1} (\log n)^3) + 2 n^{-c} (a + R).
\end{align*}
Notice that
\begin{align*}
...
\begin{align*}
	& E[|(\hat{Z}_i^T \hat{Z}_j)_{\mathrm{tr}} - Z_i^T Z_j|] \\
	\le & O(m^{-1/2} n^{-1} (\log n)^3) + 4 n^{-c} (a + R) + 2 e^{-a/R} (a + 2 m R).
\end{align*}
Let $a = m^{-1} n^{2b}$ for any $b > 0$, and $c = 2b + 3$, combined with the assumption $m = O(n^{b})$, we have
\begin{align*}
	& E[|(\hat{Z}_i^T \hat{Z}_j)_{\mathrm{tr}} - Z_i^T Z_j|] \\
	= & O(m^{-1/2} n^{-1} (\log n)^3) + O(m^{-1} n^{-3}) + O(m^{-1} n^{2b}) \cdot O(e^{-m^{-1} n^{2b}}) \\
	= & O(m^{-1/2} n^{-1} (\log n)^3) + O(m^{-1} n^{-3}) + O(m^{-1} n^{2b}) \cdot O(e^{- n^{b}}) \\
	= & O(m^{-1/2} n^{-1} (\log n)^3) + O(m^{-1} n^{-3}) + O(m^{-1} n^{2b}) \cdot O(n^{-2b-3}) \\
	= & O(m^{-1/2} n^{-1} (\log n)^3) + O(m^{-1} n^{-3}) \\
	= & O(m^{-1/2} n^{-1} (\log n)^3).
\end{align*}
\end{proof}
...
\begin{align*}
	...
	\le & O(m^{-1} n^{-2} (\log n)^6) + 2 n^{-c} E[\hat{P}_{ij}^2 \mathbb{I}\{\hat{P}_{ij} \le a\}| \mathrm{Lemma\ } \ref{lemma:1stMomentPhatDiffL1} \mathrm{\ does\ not\ hold}] \\
...
\end{theorem}
\textbf{Remark:} This is the extended version of Theorem 3.1 in \cite{oliveira2009concentration}. \\
\noindent \textbf{Require:} Lemma \ref{lemma:LqlMLE}, Theorem \ref{thm:P1Diff}\\
\noindent
\begin{proof}
Let $\{e_i\}_{i=1}^n$ be the canonical basis for $\mathbb{R}^n$. For each $1 \le i, j \le n$, define a corresponding matrix $G_{ij}$:
\[
...
\end{align*}
Based on Equation (\ref{eqn:expectpdiffqpowerkL1}), we have
\[
	E[(\hat{P}^{(1)}_{ij} - H^{(1)}_{ij})^k]
    \le E[(A^{(t)}_{ij} - H^{(1)}_{ij})^k]
    \le R^k k!,
\]
Then
\begin{align*}
...
\end{lemma}
\textbf{Remark:} This is a extended version of Proposition 4.3 in \cite{sussman2014consistent}. \\
\noindent \textbf{Require:} Theorem \ref{thm:PqDiff} and assumptions above\\
\noindent
\begin{proof}
...
with high probability.
\end{lemma}
\noindent \textbf{Require:} Theorem \ref{thm:PqDiff} and Lemma \ref{lemma:eigSShat} \\
\begin{proof}
...
with high probability.
\end{lemma}
\noindent \textbf{Require:} Theorem \ref{thm:PqDiff}, Lemma \ref{lemma:eigSShat}, Lemma \ref{lemma:AlmostOrthogonal}, Exponential distribution \\
\begin{proof}
By Proposition 2.1 in \cite{rohe2011spectral} and Equation (\ref{eqn:uhat2u2diffL1}), we have for some orthogonal matrix $W$, with high probability, 
\[
\|\hat{U} - U W\|_F^2 \le \frac{2 \|\hat{U} \hat{U}^T - U U^T\|_F^2}{\delta^2}
...
with high probability.
\end{lemma}
\noindent \textbf{Require:} Lemma \ref{lemma:AlmostOrthogonal}, Lemma \ref{lemma:exchange}\\
\begin{proof}
...
with high probability.
\end{theorem}
\noindent \textbf{Require:} Lemma \ref{lemma:XhatDiffXWexpression}, Exponential distribution \\
\begin{proof}
By Lemma \ref{lemma:XhatDiffXWexpression}, we have
\[
	\|\hat{Z} - Z W\|_F = \| (\hat{P} - H) U S^{-1/2} \|_F + O(n^{-1/2} (\log n)^{3/2})
\]
with high probability and similarly we could have the bound for each row vector
\begin{align*}
	\max_i \| \hat{Z}_i - W Z_i \|_2
...
\end{lemma}
\noindent \textbf{Require:} Theorem \ref{thm:XhatDiffXW} \\
\begin{proof}
...
\begin{align*}
	...
	\le & O(n^{-1} (\log n)^3) + a n^{-c} + (a+R) n^{-c} \\
	\le & O(n^{-1} (\log n)^3) + 2 n^{-c} (a + R).
\end{align*}
...
\[
E[|(\hat{Z}_i^T \hat{Z}_j)_{\mathrm{tr}} - Z_i^T Z_j|] = O(n^{-1} (\log n)^3).
\]
\end{proof}
...
\end{corollary}
\begin{proof}
Direct result from Theorem \ref{thm:VarLq}. {\color{red} NEED PROOF HERE.}
\end{proof}
