...
Consider a positive semi-definite stochastic blockmodel parameterized by $B \in [0,R]^{K\times K}$, and $\rho \in (0, 1)^{K}$ with $\sum_{i = 1}^K \rho_i = 1$. Note that $K$ is the number of blocks, and $R > 0$ is an upper bound for the parameter. Assuming that $\mathrm{rank}(B) = d$, then there exists $\nu \in \mathbb{R}^{K\times d}$ such that $B = \nu \nu^T$. Let $\tau$ represent the block membership of each vertex, i.e. $\tau(i) = k$ means vertex $i$ belongs to block $k$. Then the probability matrix $P \in [0, R]^{n\times n}$ can be written as $X X^T$, where $X = [X_1, \cdots, X_n]^T \in \mathbb{R}^{n\times d}$ and $X_i = \nu_{\tau_i}$.
...
Combined with the fact that $[0, R]$ is compact and the function $g(x,\theta)$ is continuous at each $\theta$ for all $x > 0$ and measurable function of $x$ at each $\theta$, we have the uniform convergence by Lemma 2.4 in \cite{newey1994large}.
\end{proof}
...
\[
\|\hat{U} - U W\|_F^2 \le \frac{2 \|\hat{U} \hat{U}^T - U U^T\|_F^2}{\delta^2}
\le \frac{8 k^2 \|\hat{U} \hat{U}^T - U U^T\|_2^2}{\delta^2} = O(n^{-1} \log n).
\]
Let $Q = \hat{U} - U U^T \hat{U}$. And $Q$ is the residual after projecting $\hat{U}$ orthogonally onto the column space of $U$, we have
\begin{equation}
\label{eqn:QFnorm}
