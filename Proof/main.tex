...
\[
	P \left( \| \hat{P}^{(1)} - H^{(1)} \|_2 \le 2 R \sqrt{n \ln(n/\eta) / m}\right) \ge 1 - \eta.
\]
\end{theorem}
\textbf{Remark:} This is the extended version of Theorem 3.1 in \href{url}{http://arxiv.org/pdf/0911.0600v2.pdf}.

Define $H^{(1)} = E[\hat{P}^{(1)}] = (1-\epsilon) P + \epsilon C$, where $P = X X^T$, $C = Y Y^T$.
Let $Z = [\sqrt{1-\epsilon} X, \sqrt{\epsilon} Y]$, then $H^{(1)} = Z Z^T$.
...
\begin{tikzpicture}[
  ...
  \draw[tofrom] (MLE) -- node[midway,left] {$\mathrm{For}$ $\mathrm{any}$ $\mathrm{fixed}$ $m$, \\$\mathrm{any}$ $1 \le i, j \le n$, \\$\underset{n \to \infty}{\lim} \mathrm{Bias}(\hat{P}_{ij}^{(1)}) =$\\$ \underset{n \to \infty}{\lim} \mathrm{Bias}(\widetilde{P}_{ij}^{(1)})$\\$\mathrm{Var}(\widetilde{P}_{ij}^{(1)})/\mathrm{Var}(\hat{P}_{ij}^{(1)})$\\$ = O(n^{-1} (\log n)^3)$} (XMLE);
  \draw[tofrom] (MLqE) -- node[midway,right] {$\mathrm{For}$ $\mathrm{any}$ $\mathrm{fixed}$ $m$, \\$\mathrm{any}$ $1 \le i, j \le n$, \\$\underset{n \to \infty}{\lim} \mathrm{Bias}(\hat{P}_{ij}^{(q)}) = $\\$\underset{n \to \infty}{\lim} \mathrm{Bias}(\widetilde{P}_{ij}^{(q)})$\\$\mathrm{Var}(\widetilde{P}_{ij}^{(q)})/\mathrm{Var}(\hat{P}_{ij}^{(q)})$\\$= O(m n^{-1} (\log n)^{3})$} (XMLqE);
...
\]
For the ML$q$E $\hat{P}^{(q)}_{ij}$,
\[
	E[\hat{P}^{(q)}_{ij}]
\]
solves a cubic equation. \RT{Shown in Mathematica}
\end{proof}
...
\]
\end{theorem}
\textbf{Remark:} See Theorem 6.2 in \href{url}{http://arxiv.org/pdf/1004.4389.pdf}.
...
\item When $k$ is even,
\[
E[X_{ij}^k] = E[(\hat{P}^{(1)}_{ij} - H^{(1)}_{ij})^k] G_{ij}^2 \preceq k! (R/m)^k G_{ij}^2;
\]
\item When $k$ is odd,
\[
E[X_{ij}^k] = E[(\hat{P}^{(1)}_{ij} - H^{(1)}_{ij})^k] G_{ij} \preceq k! (R/m)^k G_{ij}^2.
\]
\end{enumerate}
So
\[
E[X_{ij}^k] \preceq k! (R/m)^k G_{ij}^2.
\]
Let
\[
	\sigma^2 := \left\| \sum_{1 \le i < j \le n} (\sqrt{2} R G_{ij}/m)^2 \right\| = 2 R^2 \| (n-1) I \|/m^2 = 2 R^2 (n-1)/m^2,
\]
...
\begin{align*}
    ...
    \le n \exp \left( - \frac{t^2/2}{\sigma^2 + R t/m} \right) \\
    & \le n \exp \left( - \frac{t^2/2}{2 R^2 n / m^2 + R t / m} \right).
\end{align*}
...
\item When $k$ is even,
\[
E[Y_{ij}^k] = E[(\hat{P}^{(1)} - H^{(1)})^k] G_{ij}^2 \preceq k! (R/m)^k G_{ij}^2;
\]
\item When $k$ is odd,
\[
E[Y_{ij}^k] = - E[(\hat{P}^{(1)} - H^{(1)})^k] G_{ij} \preceq k! (R/m)^k G_{ij}^2.
\]
\end{enumerate}
Thus
\begin{align*}
...
\[
	t =  2 R \sqrt{n \ln(n/\eta) / m} \le 2 R n.
\]

Plugging this $t$ into the equation above, we get
\[
	P(\| \hat{P}^{(1)} - H^{(1)} \| \ge 2 R \sqrt{n \ln(n/\eta) / m})
    \le n \exp\left(-\frac{t^2}{4 R^2 n / m}\right) = \eta.
\]
\end{proof}
...
\end{lemma}
\begin{proof}
Let $\sigma_1, \cdots, \sigma_k$ denote the singular values of $U^T \hat{U}$. Then $\sigma_i = \cos(\theta_i)$ where the $\theta_i$ are the principal angles between the subspaces spanned by $\hat{U}$ and $U$. Furthermore, by the Davis-Kahan $\sin(\Theta)$ theorem, combined with Theorem \ref{thm:P1Diff} and Lemma \ref{lemma:eigSShatL1},
\begin{equation}
...
\]
for all $k \times k$ matrices $T$. Also, by Equation (\ref{eqn:uhat2u2diffL1}) we have
\begin{equation}
\label{eqn:QFnormL1}
...
\begin{align*}
    ...
    \le & k! (R/m)^k u_{is}^k u_{jt}^k \\
    \le & \frac{k!}{2} (R/m)^{k-2} (\sqrt{2} u_{is} u_{jt} R / m)^2.
\end{align*}
Also we have
\[
	\sigma^2 := |\sum_{s<t} 2 R^2 u_{is}^2 u_{jt}^2 / m^2|
    \le 2 R^2 / m^2,
\]
...
\[
	\|\hat{Z} - Z W\|_F = \| (\hat{P} - H) U S^{-1/2} \|_F + O(m^{-1/2} n^{-1/2} (\log n)^{3/2})
\]
and similarly we could have the bound for each row vector \RT{$k$ should be $d$ here?}
\begin{align*}
...
\]
Thus, conditioned on $H$, the $i$-th element of the vector $(\hat{P} - H) u_j$ is a sum of independent mean 0 random variables and a term of order $O(1)$. \RT{We don't have $O(m^{-1})$ here!}
By Equation \ref{eqn:expectpdiffqpowerk}, we have
\begin{align*}
	& E\left[\left((\hat{P}_{is} - H_{is}) u_{js}\right)^k\right] \\ 
    \le & k! (R/m)^k u_{js}^k \\
    \le & \frac{k!}{2} (R/m)^{k-2} (\sqrt{2} R u_{js} / m)^2.
\end{align*}
Also we have
\[
	\sigma^2 := |\sum_{s \ne i} 2 R^2 u_{js}^2 / m^2|
    \le 2 R^2 / m^2,
\]
then by Theorem \ref{thm:BernsteinMatrix}, we have
\[
    ...
    \le \exp \left( \frac{-t^2/2}{2 R^2 / m^2 + R t / m} \right),
\]
i.e. it is of order $O(m^{-1})$ with high probability.
Taking the union bound over all $i$ and $j$, we have
\begin{align*}
	\max_i \| \hat{Z}_i - W Z_i \|_2
    & \le \frac{C k^{1/2}}{\lambda_k^{1/2}(H)} m^{-1/2} \log n + O(m^{-1/2} n^{-1/2} (\log n)^{3/2}) \\
...
From now on, we will use $\hat{P}$ to represent $\hat{P}^{(q)}$, use $H$ to represent $H^{(q)}$. Assume $H = U S U^T = Z Z^T$, where $Z$ is a $n$-by-$k$ matrix. Then our estimate for $Z$ up to rotation is $\hat{Z} = \hat{U} \hat{S}^{1/2}$, where $\hat{U} \hat{S} \hat{U}^T$ is the rank-$k$ spectral decomposition of $|\hat{P}| = (\hat{P}^T \hat{P})^{1/2}$.
...
\]
thus each entry of $U^T(\hat{P} - H)U$ is of order $O(1)$ with high probability and thus 
\begin{equation}
\label{eqn:uPhatdiffHu}
	\|U^T(\hat{P} - H)U\|_F = O(1)
\end{equation}
with high probability.
\RT{Does it make sense here to be $O(1)$?}
Hence
\[
	\| W^* \hat{S} - S W^* \|_F = O(\log n)
\]
