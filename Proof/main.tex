...
Define $H^{(1)} = E[\hat{P}^{(1)}] = (1-\epsilon) P + \epsilon C$, where $P = X X^T$, $X \in \mathbb{R}^{n \times d}$, $C = Y Y^T$, $Y \in \mathbb{R}^n$. \RT{Extend $C$ to be rank $d'$?}
...
Since $H^{(1)} = [\sqrt{1-\epsilon} X, \sqrt{\epsilon} Y] [\sqrt{1-\epsilon} X, \sqrt{\epsilon} Y]^T$, we have $d^{(1)} \le d+1$.


For simplicity, from now on, we will use $\hat{P}$ to represent $\hat{P}^{(1)}$, use $H$ to represent $H^{(1)}$ and use $k$ to represent the dimension $d^{(1)}$ we are going to embed. Assume $H = U S U^T = Z Z^T$, where $Z$ is a $n$-by-$k$ matrix. Then our estimate for $Z$ up to rotation is $\hat{Z} = \hat{U} \hat{S}^{1/2}$, where $\hat{U} \hat{S} \hat{U}^T$ is the rank-$d$ spectral decomposition of $|\hat{P}| = (\hat{P}^T \hat{P})^{1/2}$.
...
\end{equation}\RT{$\lambda_k(H) \ge n$, here we need assumption of eigengap?}
...
\end{align*}
where $u_i$ and $u_j$ are the $i$-th and $j$-th columns of $U$. Thus, conditioned on $H$, $U$ is fixed and $u_i^T (\hat{P} - H) u_j$ is a sum of independent mean 0 random variables and a term of order $O(1)$.
...
\]
thus each entry of $U^T(\hat{P} - H)U$ is of order $O(1)$ with high probability and thus \RT{$O(1)$ here?}
\begin{equation}
...
\]
i.e. it is of order $O(1)$ with high probability. \RT{$O(1)$ here?}
Taking the union bound over all $i$ and $j$, we have
\begin{align*}
	\max_i \| \hat{Z}_i - W Z_i \|_2
    & \le \frac{C k^{1/2}}{\lambda_k^{1/2}(H)} \log n + O(n^{-1/2} (\log n)^{3/2}) \\
    & = O(n^{-1/2} (\log n)^{3/2}).
\end{align*}
\RT{$\log n$}
\end{proof}
