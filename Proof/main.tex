...
For simplicity, from now on, we will use $\hat{P}$ to represent $\hat{P}^{(1)}$, use $H$ to represent $H^{(1)}$ and use $d$ to be the dimension $d^{(1)}$ we are going to embed. Assume $H = U S U^T = Z Z^T$, where $Z$ is a $n$-by-$d$ matrix. Then our estimate for $Z$ up to rotation is $\hat{Z} = \hat{U} \hat{S}^{1/2}$, where $\hat{U} \hat{S} \hat{U}^T$ is the rank-$d$ spectral decomposition of $|\hat{P}| = (\hat{P}^T \hat{P})^{1/2}$.
...
\label{eqn:uhat2u2diffL1}
    ...
    \le \frac{\|\hat{P} - H\|_2}{\lambda_d(H)}
    \le \frac{C \sqrt{n \log n}}{n} = O(n^{-1/2} \sqrt{\log n})
\end{equation}\RT{$\lambda_d(H) \ge n$?}
for sufficient large $n$. Here $\lambda_d(H)$ denotes the $d$-th largest eigenvalue of $H$.\\
We thus have
\begin{align*}
	\| U^T \hat{U} - W_1 W_2^T \|_F
    & = \| \Sigma - I \|_F
    = \sqrt{\sum_{i=1}^d (1-\sigma_i)^2} \\
    & \le \sum_{i=1}^d (1-\sigma_i) \le \sum_{i=1}^d (1-\sigma_i^2) \\
    & = \sum_{i=1}^d \sin^2(\theta_i)
    \le d \|\hat{U} \hat{U}^T - U U^T\|_2^2 \\
    & = O(n^{-1} \log n).
\end{align*}
\RT{First inequality?}
\end{proof}
