...
for (m in mVec) {
  ...
  fileName = paste0("../../Result/result_compareLR_brute_", dataName, "_",
                    strWeighted, "_", "m_", m, "_q_", q, "_", strSVD, ".RData")
  ...
  if (!(file.exists(fileName))) {
    ...
    out <- mclapply(1:nIter, function(x) ExpAllDimCompareLR(AList, m, q, isSVD, dVec),
                    mc.cores=nCores)
    out <- array(unlist(out), dim = c(2*nD+11, nIter))
    ...
    save(errorMLE, errorMLEASE, errorMLqE, errorMLqEASE,
         ...
         dimP, n, M, m, dVec, isWeighted, nIter, file=fileName)
  }
}
