...
for (m in mVec) {
  ...
  out <- mclapply(1:nIter, function(x) ExpAllDimTruncate(M, m, dVec, AList, ASum, q, isSVD), 
                  mc.cores=nCores)
  ...
  if (isSVD) {
    fileName = paste("../../Result/result_", dataName, "_truncate_brute_", "m_", m, "_q_", q, "_svd.RData", sep="")
  } else {
    fileName = paste("../../Result/result_", dataName, "_truncate_brute_", "m_", m, "_q_", q, "_eig.RData", sep="")
  }
