...
for (i in 1:M) {
  tmp <- eigen(AList[[i]])
#   tmp <- eigen(diag_aug(AList[[i]]))
  eigenVec[i] <- tmp$values[n]
}
hist(eigenVec)
boxplot(eigenVec)

# tmp <- eigen(ASum/M)
# eigenExtra <- max(-tmp$values[n], 0)
eigenExtra <- max(-sort(eigenVec)[2], 0)

for (i in 1:M) {
  tmp <- eigen(AList[[i]])
  AList[[i]] <- AList[[i]] + diag(rep(eigenExtra, n))
}
...
for (m in mVec) {
  ...
  out <- mclapply(1:nIter, function(x) ExpAllDimAug(M, m, dVec, AList, ASum, q, isSVD), 
                  mc.cores=nCores)
  ...
  if (isSVD) {
    fileName = paste("../../Result/result_", dataName, "_brute_", "m_", m, "_q_", q, "_svd_aug.RData", sep="")
  } else {
    fileName = paste("../../Result/result_", dataName, "_brute_", "m_", m, "_q_", q, "_eig_aug.RData", sep="")
  }
